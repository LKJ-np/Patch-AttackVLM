...
if __name__ == "__main__":
    ...
    # parser.add_argument("--clip_encoder", default="ViT-B/32", type=str)  RN50', 'RN101', 'ViT-B/16', 'ViT-B/32', 'ViT-L/14
    ...
    parser.add_argument("--output", default="../tag_Adam_Out/Adam_100_zuo23/", type=str,help='')
    parser.add_argument("--cle_data_path", default='../data/test_cle1/',type=str, help='')
    parser.add_argument("--tgt_data_path",default='../data/test_tag1/',type=str, help='')
    ...
    for i, ((image_org, _, path), (image_tgt, _, _)) in enumerate(zip(data_loader_imagenet, data_loader_target)):
        ...
        adv_patch_cpu = torch.rand((1, 3, 32,32))
        ...
        image_org /= 255
        args.nps_calculator = NPSCalculator("../non_printability/30values.txt",32).cuda()
        args.total_variation = TotalVariation().cuda()
        midle_embedding_sim=1
        adv_patch_tmp = torch.rand((1, 3, 32, 32))
        for j in range(args.steps):
            ...
            embedding_sim = torch.mean(torch.sum(adv_image_features * tgt_image_features,
            ...
            # if embedding_sim < max_similarity:
            #     max_similarity=embedding_sim
            #     loss += max_similarity
            # else:
            #     loss += embedding_sim
            # embedding_sim += nps_loss
            ...
            if embedding_sim < midle_embedding_sim:
                adv_patch_tmp = adv_patch_cpu
                midle_embedding_sim = embedding_sim
                print(j)
            print(
        ...
        # save imgs
        # _show_images(adv_patch_cpu)
        adv_patch_cpu = adv_patch_tmp
